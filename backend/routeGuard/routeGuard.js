...
const isAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (userPosition === 'MA' || userPosition.toLowerCase().includes('admin')) {
...
const checkAdminStatus = async (req, res) => {
  try {
    ...
    const userPosition = result.recordset[0].position;
    const isAdminUser = userPosition === 'MA' || userPosition.toLowerCase().includes('admin');
